...
### Phase 4: Provider-Agnostic Routing (Abstracted Capabilities)
NeuroGenesis treats foundation models not as static text strings (e.g., "claude-3.5-sonnet") but as **Capability Pools**. 

**Why this elevates the experts:** 
Hardcoding an agent to a specific model limits its intelligence to whatever was SOTA on the day it was written. By abstracting capabilities (e.g., `capability: deep-reasoning`, `capability: massive-context`, `capability: fast-execution`), the OS can dynamically route tasks to the best available model in the user's environment. 
*   If Anthropic releases Claude 4 tomorrow, the user updates their central provider config, and *every* expert agent requiring `deep-reasoning` instantly receives an IQ upgrade without touching a single prompt.
...
To solve the "Provider Lock-in" problem where AI tools hardcode specific model strings (e.g., `gpt-4o`), NeuroGenesis uses **Omni-Bind**.

**The Omni-Bind Resolver:**
...
7. **Strict Anti-Simulation (No Hallucinated Personas):** The orchestrating agent (e.g., the Lead Agent) is strictly forbidden from simulating, roleplaying, or hallucinating the responses of other agents or panels. The system's competitive advantage ("secret sauce") relies on routing specific cognitive profiles to the absolute best, highly-tuned foundation model for that exact role. Simulating a specialist locally destroys this advantage and degrades the review quality to the current model's baseline. If a rule requires the input of the Security Sentinel or a full panel, the Lead Agent MUST physically invoke those distinct agents via the system's designated multi-agent tooling (e.g., the `Task` tool). 
    *   **Failure Protocol:** If there is a technical issue spawning the required agent (e.g., tool error, API timeout), the Lead Agent MUST immediately halt and inform the user. It must explicitly warn the user that simulating the expert locally will negatively affect the quality and safety of the review, and it must ask for explicit human permission before proceeding with a simulated fallback. True, independent execution of the specialist agent is the standard operating requirement.
