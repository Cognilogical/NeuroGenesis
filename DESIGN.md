...
**The Template-and-Render Compiler:**
Instead of fragile symlinks, NeuroGenesis uses a multi-format rendering engine. It reads the canonical `.neurogenesis/lead_agent.json` and compiles it into the specific formats required by different tools:
*   `AGENTS.md` (Markdown format for OpenCode / deepwiki-rs)
*   `.github/copilot-instructions.md` (Markdown format for Copilot CLI/IDE)
*   `.cursorrules` / `.windsurfrules` (Markdown format for Cursor/Windsurf)
*   `.clauderc` (JSON/TOML format for Anthropic's Claude Code CLI)

Each rendered file includes a header warning (`# AUTO-GENERATED by NeuroGenesis — do not edit`). This guarantees that no matter what tool the developer opens the repository in, the AI wakes up bound to the exact same project-specific constraints and rules of engagement.
